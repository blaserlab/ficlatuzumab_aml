...
gene_name_list<-list(mod7genes)
list_name_list<-c("mod7")
...
new_agg_cds_mod7 <- new_agg_cds


# agg_gene_fits_mod7 <- fit_models(new_agg_cds_mod7[rowData(new_agg_cds_mod7)$id == "mod7",
#                                         colData(new_agg_cds_mod7)$treatment_day1=="0"],
#                             model_formula_str = "~response+aligned_partition")
# 
# agg_fit_coefs_mod7 <- coefficient_table(agg_gene_fits_mod7)
# agg_fit_coefs_mod7

mod7_monocle_regression <- lapply(
  X = seq_along(treatment_days),
  FUN = monocle_regression_func,
  cds = new_agg_cds_mod7,
  gene_or_genes = "mod7",
